...
pipeline {
    ...
    environment {
        ...
        SLACK_WEBHOOK_URL = credentials('slackwebhooktoken')
        // Email recipients - update these with your actual email addresses
        EMAIL_RECIPIENTS = 'mlnarasimhaiah@gmail.com, dnarasimha1994@gmail.com'
    }
            ...
            steps {
                ...
                script {
                    env.DEV_TEST_STATUS = sh(
                        script: 'npx playwright test --grep "@smoke" --config=playwright.config.dev.ts',
                        returnStatus: true
                    ) == 0 ? 'success' : 'failure'
                }
            ...
            steps {
                ...
                script {
                    env.STAGE_TEST_STATUS = sh(
                        script: 'npx playwright test --grep "@smoke" --config=playwright.config.stage.ts',
                        returnStatus: true
                    ) == 0 ? 'success' : 'failure'
                }
            ...
            steps {
                ...
                script {
                    env.PROD_TEST_STATUS = sh(
                        script: 'npx playwright test --grep "@smoke" --config=playwright.config.prod.ts',
                        returnStatus: true
                    ) == 0 ? 'success' : 'failure'
                }
